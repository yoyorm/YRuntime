...
| D4 | 自研最小数学库，GLM 只在 Vulkan 后端用 | 暂定 | M1 |
...
**后果**：+ 各自的性能与安全都最优；− 概念数量 +1，新人（含 3 个月后的你）容易混 → 必须在 `01-architecture.md` 与两篇设计笔记里各画一张对照表。

**待定子问题**：generation 位宽（40 bit 是否浪费？32/32 是否够？）、`Handle<T>` 是否允许 `Handle<const T>`、哈希函数选择 → M1 用 benchmark E1 + 测试回绕来决定。

---

## D4 · 自研最小数学库，GLM 只在 Vulkan 后端用
**状态**：暂定（对应 `01-architecture.md` Q1）· **里程碑**：M1
...
**后果**：− M1 多花约 3~5 天；− M10 需要 `Mat4 ↔ glm::mat4` 转换函数（写一次，加测试）；+ Runtime 编译更快、依赖更少。
...
实现顺序：**先手写一个不用宏的 `ClassInfo` 注册版本并跑通测试 → 再封装成宏 → 用 `clang -E` 展开验证 → 把展开结果贴进设计笔记**。
...
**先写格式规范文档 `notes/design/scene-format.md`，再写解析器。**
...
benchmark 放 `benchmarks/` 独立 target，**手动运行**，结果写 `doc/notes/benchmarks/`。
