...
| **费曼式验收** | 每个模块完成后，对着空白纸讲 5 分钟（可录音），讲不下去的地方就是没懂 | `notes/_design-note-template.md` §9 就是这个机制 |
| **用实验回答问题** | 每个"我不确定 X 是否重要"的疑问 → 写一个 benchmark 或最小复现，用数据回答（见 §4） | 引擎开发的判断力来自测量，不来自直觉或博客 |
| **写博客倒逼理解** | 每完成一个里程碑，写一篇 1500 字左右的技术文章（`07-portfolio.md` §4） | 写不出来 = 没懂；写出来 = 作品 |
...
**读源码的时间盒**：单次 ≤60 分钟，读完必须写 3 行结论进笔记。读不出结论就是读错了文件。
...
每个实验产出 `doc/notes/benchmarks/<名字>.md`：问题 / 方法 / 数据表 / 结论 / 对设计的影响。

| # | 实验 | 对应 M | 要回答的问题 |
|---|---|---|---|
| E1 | SlotMap vs `unordered_map<uint32_t,T>` vs `vector<T>+freelist` | M1 | 插入/查找/遍历各差多少？cache-miss 差多少（`perf stat`）？ |
| E2 | `SparseSet` vs `unordered_map` 做组件存储 | M1/S3 | 稠密迭代收益；稀疏度多高时收益消失？ |
